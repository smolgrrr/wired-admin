...
class AdminFiniteRelaySession implements FiniteRelaySession {
  ...
  query(input: FiniteRelayQuery): Promise<QueryCompletion> {
    ...
    const finish = (reason: QueryCompletion["reason"]) => {
      ...
      const unfinishedState: RelayCompletionState =
        reason === "cancelled" ? "cancelled" : "timed-out";
...
export async function withFiniteRelaySession<T>(
  ...
  const session = new AdminFiniteRelaySession();

...
export type RelayTranscriptEntry =
  ...
  | (TranscriptBase & { type: "publish"; eventId: string; bytes: number })
  | (TranscriptBase & {
...
export class RelayTranscriptSession {
  ...
  summary(workflow: RelayWorkflowCapture): RelayTranscriptSummary {
    ...
    return {
      ...
      openedConnections: entries.filter((entry) => entry.type === "connection-opened").length,
      closedConnections: entries.filter((entry) => entry.type === "connection-closed").length,

...
This runbook validates one 21-sat NIP-57 zap and its 14-sat creator payout on
...
- Blink balance before the test: at most 79 sats, keeping the balance at or below
  the 100-sat cap after receiving the 21-sat zap. Do not add funds merely to make
...
- One new Wired staging post, one saved creator Lightning address, one 21-sat
  zap, and one expected 14-sat payout.
- `STAGING_REVENUE_MAX_SENDABLE_MSAT=21000`,
  `STAGING_REVENUE_MINIMUM_PAYOUT_MSAT=14000`, and
...
At 21 sats, the ledger should allocate 14,700 msat to the creator and 6,300
msat to Wired. Blink pays the whole-sat creator portion (14 sats); the remaining
700 msat stays available for later aggregation. The routing fee is charged to
...
- a request above 21 sats produces an invoice;
- the 21-sat invoice does not settle within two minutes after payment;
- no valid kind-9735 receipt is published within two reconciliation cycles;
- the creator/Wired split differs from 14,700/6,300 msat;
...
1. Before deployment, run this read-only Blink query with the staged API key and
   ...
   `walletCurrency: BTC`, balance no greater than 79 sats, and exactly the needed
...
4. Zap that post exactly 21 sats from a separate wallet.
5. In Blink, record the incoming invoice payment hash, amount, and paid status.
6. Reconcile Wired and verify one 21,000-msat settled invoice, a valid public
   receipt, 14,700 msat creator-available, 6,300 msat Wired revenue, and no
   payout transaction.
...
3. Verify the creator received exactly 14 sats and the 700-msat remainder is
...
4. Match Wired's provider payment ID, amount, final status, and fee to Blink's
   ...
   respectively 700/0/14,000 msat.
...
14,700 msat creator liability reserved or available in Wired's durable ledger,
